...
% Đặt tên caption bảng bằng tiếng Việt cho chương này.
...
Luồng xử lý tổng quát của hệ thống gồm các bước chính sau:
...
\hline
5 & Kiểm tra năng lượng & Robot chỉ chọn mục tiêu nếu năng lượng còn lại đủ để đi tới mục tiêu, quay về căn cứ và giữ phần dự phòng. \\
\hline
...
Có thể xem hệ thống như một bộ điều khiển nhiệm vụ theo vòng lặp. Ở mỗi bước, robot không quyết định chỉ dựa trên mục tiêu gần nhất, mà còn xét chi phí đường đi, năng lượng còn lại, đường quay về căn cứ và trạng thái an toàn của môi trường. Điều này giúp mô phỏng phản ánh rõ hơn các ràng buộc thường gặp trong một nhiệm vụ dài, đặc biệt là khi robot hoạt động trong khu vực có rủi ro hoặc có các đối tượng chuyển động.

Các thành phần chính của hệ thống được tóm tắt trong Bảng 3.1.
...
\hline
Bộ xử lý năng lượng & Tính năng lượng di chuyển, năng lượng quay, năng lượng dự phòng và điều kiện quay về căn cứ. \\
\hline
...
Dữ liệu đầu vào của một bản đồ gồm các thành phần sau:
...
\hline
Tùy chọn Metric H & Nếu được bật, hệ thống bỏ qua chi phí quay để tạo mô hình đối chứng. \\
\hline
\end{tabular}
...
Gọi bản đồ có $m$ hàng và $n$ cột. Mỗi ô được biểu diễn bởi cặp tọa độ:

\[
v=(r,c),\quad 1 \le r \le m,\quad 1 \le c \le n.
\]

Tại mỗi ô, hệ thống lưu các thông tin chính:
...
Căn cứ của robot được ký hiệu là $b$. Trong hệ thống, căn cứ chính là ô xuất phát của robot:
...
Robot trong hệ thống được mô phỏng ở mức lập kế hoạch và điều khiển rời rạc trên bản đồ lưới. Tại thời điểm $t$, trạng thái chính của robot có thể được biểu diễn như sau:
...
Tập hướng của robot gồm bốn hướng chính:

\[
H=\{N,E,S,W\}.
\]

Cách rời rạc hóa bốn hướng được chọn vì phù hợp với bản đồ lưới và giúp hệ thống mô phỏng ổn định. Các mô hình nhiều hướng hơn có thể được xây dựng bằng cách mở rộng tập $H$ và bổ sung chi phí chuyển hướng tương ứng, nhưng điều đó làm tăng số trạng thái mà bộ lập kế hoạch phải xử lý.

Do đó, một trạng thái dùng cho lập kế hoạch đường đi có dạng:
...
Trạng thái nhiệm vụ của robot được mô tả bằng các mode chính trong Bảng 3.3.
...
Khi robot đi từ ô $v$ sang ô kề cạnh $v'$, năng lượng di chuyển chuẩn hóa được tính bằng chi phí đi vào ô $v'$:

\[
E_{\text{move}}(v,v') = c_{\text{eff}}(v').
\]

Ở đây $c_{\text{eff}}(v')$ là chi phí hiệu dụng của ô $v'$ tại thời điểm lập kế hoạch. Nếu ô $v'$ là vật cản tĩnh hoặc đang bị vật cản động chiếm trong chế độ xét vật cản động, ta có:

\[
c_{\text{eff}}(v') = \infty.
\]

Việc gán chi phí vô cùng lớn tương đương với việc loại ô đó khỏi đồ thị tại thời điểm lập kế hoạch. Cách biểu diễn này thuận tiện cho cài đặt vì hệ thống vẫn giữ nguyên ma trận bản đồ, nhưng chỉ cần thay đổi chi phí hiệu dụng của ô tùy theo trạng thái vật cản tĩnh hoặc vật cản động.

Ngược lại, nếu ô $v'$ hợp lệ và an toàn tại thời điểm xét, ta có:

\[
c_{\text{eff}}(v') = c(v').
\]

Chi phí quay được tính theo số lần quay 90 độ cần thiết để chuyển từ hướng hiện tại $h$ sang hướng tiếp theo $h'$. Gọi:

\[
\Delta(h,h') \in \{0,1,2\}
\]

là số lần quay 90 độ nhỏ nhất giữa hai hướng. Trong mô hình mặc định, chi phí cho một lần quay 90 độ tại ô $v$ là:

\[
\tau(v)=\frac{1}{2}c(v).
\]

Giá trị này tương ứng với chuẩn hóa $L=2s/\pi$ và $\kappa_{\text{turn}}=1$ đã trình bày ở Chương 2. Nói cách khác, trong cấu hình mặc định, một lần quay 90 độ được xem là tiêu tốn bằng một nửa chi phí di chuyển qua một ô trên cùng địa hình. Nếu xét robot có tỉ lệ hình học hoặc đặc điểm đổi hướng khác, hệ thống có thể thay đổi hệ số chi phí quay mà không phải thay đổi cấu trúc thuật toán Dijkstra có hướng.

Do đó, chi phí chuyển từ trạng thái $(v,h)$ sang trạng thái $(v',h')$ là:
...
Trong Metric H, chi phí quay được bỏ qua:
...
Bảng 3.4 tóm tắt các quy tắc chi phí được sử dụng trong mô phỏng.
...
\hline
\textbf{Thao tác} & \textbf{Mô hình mặc định} & \textbf{Metric H} \\
\hline
...
Bộ lập kế hoạch của hệ thống sử dụng Dijkstra có hướng. Thay vì tìm đường trên tập ô $V_{\text{free}}$, thuật toán tìm đường trên không gian trạng thái mở rộng:
...
Khoảng cách tối ưu tới một trạng thái được lưu dưới dạng:

\[
d(r,c,h).
\]

Trạng thái bắt đầu có chi phí bằng 0:

\[
d(r_0,c_0,h_0)=0.
\]

Khi xét một bước chuyển từ trạng thái $q$ sang trạng thái $q'$, thuật toán cập nhật:

\[
d(q') \leftarrow \min\bigl(d(q'),\ d(q)+w_q(q,q')\bigr).
\]

Pseudo-code của Dijkstra có hướng trong hệ thống có thể mô tả như sau:
...
Hệ thống lưu thêm bảng truy vết để khôi phục đường đi sau khi tìm được mục tiêu. Khi cần đường đi tới một ô đích, hệ thống không cố định trước hướng đến đích. Thay vào đó, nó xét bốn hướng có thể tại ô đích và chọn hướng có chi phí nhỏ nhất:
...
Tại mỗi thời điểm cần lập kế hoạch mới, hệ thống xác định tập các ô chưa bao phủ:

\[
U_t=\{v\in V_{\text{free}} \mid \text{covered}_t(v)=false\}.
\]

Không phải mọi ô trong $U_t$ đều có thể được chọn làm mục tiêu ngay lập tức. Một ô chỉ được xem là ứng viên nếu robot có đường đi tới ô đó theo trạng thái hiện tại và đường đi này có chi phí hữu hạn. Với mỗi mục tiêu ứng viên $g$, hệ thống tính hai đại lượng:
...
Hệ thống sắp xếp các ứng viên theo chi phí đi tới mục tiêu. Sau đó, lần lượt kiểm tra từng ứng viên theo điều kiện năng lượng. Gọi $M(d)$ là năng lượng dự phòng cần giữ lại cho đường quay về có chi phí $d$. Mục tiêu $g$ được xem là khả thi với năng lượng hiện tại nếu:

\[
E_t \ge d_{\text{to}}(g)+d_{\text{return}}(g)+M(d_{\text{return}}(g)).
\]

Trong mô phỏng, $M(d)$ được chọn theo nguyên tắc có một mức dự phòng tối thiểu và có thể tăng theo độ dài đường quay về. Cụ thể, hệ thống sử dụng mức dự phòng tối thiểu 15 đơn vị năng lượng và một thành phần tỷ lệ với chi phí quay về. Khi đường quay về dài hơn, robot cần thận trọng hơn vì khả năng gặp vật cản động hoặc phải lập kế hoạch lại cũng tăng lên.

Pseudo-code của bước chọn mục tiêu có thể viết như sau:
...
Điểm quan trọng của chiến lược này là robot không chọn mục tiêu chỉ vì mục tiêu đó gần nhất theo số bước. Mục tiêu được chọn phải đồng thời thỏa mãn ba điều kiện: có đường đi tới, có đường quay về và đủ năng lượng cho cả hai chặng kèm dự phòng. Cách chọn này giúp hạn chế tình huống robot đạt thêm một phần bao phủ trước mắt nhưng làm suy giảm khả năng quay về căn cứ.

Chiến lược trên thuộc nhóm chọn mục tiêu tham lam có ràng buộc. Hệ thống không tìm toàn bộ thứ tự bao phủ tối ưu cho cả nhiệm vụ, mà chọn mục tiêu khả thi tốt nhất tại thời điểm lập kế hoạch hiện tại. Lựa chọn này giúp quá trình mô phỏng ổn định, dễ kiểm tra và phù hợp với mục tiêu đánh giá tác động của chi phí quay, trọng số địa hình, năng lượng và vật cản động.
...
Tại vị trí hiện tại $v_t$, hệ thống ước lượng chi phí quay về căn cứ:

\[
d_b = E(P_{v_t\rightarrow b}).
\]

Robot được yêu cầu quay về nếu năng lượng còn lại nhỏ hơn hoặc bằng chi phí quay về cộng với năng lượng dự phòng:

\[
E_t \le d_b + M(d_b).
\]

Trong hệ thống, mức dự phòng được tính theo công thức khái quát:

\[
M(d_b)=\max\left(M_{\min},\ M_{\text{prop}}(d_b)\right),
\]

trong đó $M_{\min}$ là dự phòng tối thiểu và $M_{\text{prop}}(d_b)$ là thành phần dự phòng tăng theo chi phí quay về. Với cấu hình hiện tại, $M_{\min}=15$ và thành phần tỷ lệ được lấy xấp xỉ bằng $25\%$ chi phí quay về, tức:
\[
...
Công thức dự phòng này là một chính sách cụ thể của hệ thống, không phải lựa chọn duy nhất có thể có. Các chính sách khác như dự phòng cố định, dự phòng theo phần trăm pin hoặc dự phòng theo mức rủi ro môi trường đều có thể được quy về một hàm $M(d)$. Trong phạm vi triển khai hiện tại, dạng $\max(M_{\min},M_{\text{prop}})$ được chọn vì dễ kiểm soát và phản ánh trực giác rằng đường quay về càng dài thì robot càng nên giữ mức dự phòng lớn hơn.

Robot rơi vào vùng năng lượng tới hạn nếu:
...
Trong cấu hình hiện tại, $E_{\min}=3$. Trạng thái tới hạn không được hiểu là hệ thống lập tức thất bại trong mọi trường hợp. Nó là tín hiệu để robot chuyển sang các cơ chế ứng phó thận trọng hơn như chờ, tìm đường vòng, quay về nếu có thể, hoặc bảo toàn trạng thái nhiệm vụ.

Khi robot về tới căn cứ, nếu nhiệm vụ chưa hoàn thành, robot chuyển sang trạng thái sạc. Sau khi sạc đầy, robot quay lại trạng thái bao phủ bình thường và tiếp tục xử lý các ô chưa bao phủ. Nếu toàn bộ bản đồ đã được bao phủ và robot đã ở căn cứ, nhiệm vụ được xem là hoàn thành đầy đủ.
...
Tại thời điểm $t$, gọi $O_t$ là tập ô đang bị vật cản động chiếm dụng. Khi đó, tập ô an toàn là:

\[
S_t=V_{\text{free}}\setminus O_t.
\]

Một bước đi tới ô $v'$ chỉ được phép bắt đầu nếu:
...
Vật cản động trong hệ thống gồm hai nhóm chính:
...
Mô phỏng được tổ chức theo các bước thời gian rời rạc. Ở mỗi bước, hệ thống cập nhật trạng thái robot, trạng thái vật cản động, đường đi hiện tại, năng lượng và dữ liệu hiển thị. Vòng lặp chính có thể được mô tả như sau:
...
Những chuyển trạng thái này không nhằm tạo ra một mô hình điều khiển robot thực địa hoàn chỉnh. Chúng là một chính sách mô phỏng ở mức nhiệm vụ, giúp robot có hành vi nhất quán khi gặp các ràng buộc về năng lượng, vật cản động và điều kiện quay về.
...
Các chỉ số chính được hệ thống ghi nhận gồm:
...
Chương này cũng đã mô tả chiến lược chọn mục tiêu bao phủ có xét điều kiện quay về căn cứ, chính sách năng lượng dự phòng, cơ chế sạc, xử lý vật cản động, vòng lặp mô phỏng, các trạng thái nhiệm vụ và hệ thống ghi log, thống kê. Các thành phần này tạo thành một hệ thống mô phỏng hoàn chỉnh, có khả năng chạy thử, quan sát và thu thập dữ liệu để đánh giá. Trên cơ sở đó, Chương 4 sẽ phân tích sâu hơn các đặc điểm lý thuyết của phương pháp, bao gồm tác động của chi phí quay, điều kiện an toàn năng lượng, hành vi trong môi trường động và độ phức tạp của các bước lập kế hoạch chính.
