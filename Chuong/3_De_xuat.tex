...
\]
Trong phiên bản hiện tại, $M(d)$ được áp dụng theo loại kịch bản:
\[
M(d)=
\begin{cases}
...
\]
Robot được yêu cầu quay về nếu năng lượng còn lại nhỏ hơn hoặc bằng chi phí quay về cộng với năng lượng dự phòng của kịch bản:
\[
E_t \le d_b + M(d_b).
\]
Trong hệ thống, hàm dự phòng được tính như sau:
\[
M(d_b)=
\begin{cases}
0, & \text{khi kịch bản không có vật cản động},\\
...
\end{cases}
\]
Trong nhánh có vật cản động, $M_{\min}$ là dự phòng tối thiểu và $M_{\text{prop}}(d_b)$ là thành phần dự phòng tăng theo chi phí quay về. Với cấu hình hiện tại, $M_{\min}=15$ và thành phần tỷ lệ được lấy xấp xỉ bằng $25\%$ chi phí quay về, tức:
\[
